...
- How much storage data should we expose only for testing purposes? Currently making `purposeCount` and others public only for purpose of testing.
- How to test parameter values in `truffleAssert.eventEmitted`? Using `{ param3: target }` gives `AssertionError: Event filter for PurposeUpdated returned no results`.
